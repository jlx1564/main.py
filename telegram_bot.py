...
# Telegram API Token
API_TOKEN = 7980626247AAEtckoqoNQEUEVp7GT9I6SRMA-SN86TcF0
CHANNEL_ID = @piterfr

# URL для парсинга
TIMEPAD_URL = httpsafisha.timepad.rusaint-petersburgsearchprice=free
...
async def fetch_events()
    ...
    for event in soup.select(article.eventpage)  # Основной контейнер события
        ...
        event_text = (f📅 Дата и время {date_time}n
                      f📍 Место {location}n
                      f📝 {title}n
                      f🔗 [Регистрация]({link}))
...
async def post_events()
    ...
    for event in events
        try
            ...
            logging.error(fОшибка при отправке {e})
